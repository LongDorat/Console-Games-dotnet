...
class Program
{
    ...
    const double MAX_MINE_RATIO = 0.99;
    const double MIN_MINE_RATIO = 0.01;
    ...
    static void Main(string[] args)
    {
        ...
        double mineRatio = GetMineRatioInput("Enter the mine ratio (0-1): ", MIN_MINE_RATIO, MAX_MINE_RATIO);
    ...
    static public void EndGame(string message, Cell[,] board, Cursor cursor)
    {
        cursor.Row = board.GetLength(0);
        cursor.Column = 0;
        Console.Clear();
        Render.Board(board, cursor);
...
static class Control
{
    ...
    {
        //? If the player steps on a mine, reveal all the cells and end the game
        if (board[cursor.Row, cursor.Column].IsMine && !board[cursor.Row, cursor.Column].IsFlagged)
        {
            ...
            Program.EndGame("You lost!", board, cursor);
        }

        //? If the player steps on a cell with no adjacent mines, reveal all the adjacent cells
